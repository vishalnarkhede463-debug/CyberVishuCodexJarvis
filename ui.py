...
from PyQt6.QtWidgets import (
    QApplication, QDialog, QFileDialog, QFrame, QGraphicsDropShadowEffect,
...
class HudCanvas(QWidget):
    def __init__(self, face_path: str, assistant_name: str = "CyberVishuCodex", parent=None):
...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._pos     = 0
        self._tag     = "sys"
        self._ai_name_lc = "cybervishucodex"   # updated when assistant name changes
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for CyberVishuCodex", str(Path.home()),
            "All Files (*.*);;"
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="CyberVishuCodex", user_name="",
    ...
    def _save(self):
        name = self._name_input.text().strip() or "CyberVishuCodex"
...
class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied — offers quick CyberVishuCodex actions."""
...
class RemoteKeyOverlay(QWidget):
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected — CyberVishuCodex ready")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "CyberVishuCodex").strip()
    ...
    def _build_header(self) -> QWidget:
        ...
        _sub_text = ("Just A Rather Very Intelligent System"
                     if _disp in ("JARVIS", "J.A.R.V.I.S", "CYBERVISHUCODEX")
    ...
    def _open_customize(self):
        ...
        cw = self.centralWidget()
        ov = CustomizeOverlay(
            cfg.get("assistant_name", "CyberVishuCodex") or "CyberVishuCodex",
            cfg.get("user_name", ""),
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = ""):
        ...
        self._assistant_name = name.strip() or "CyberVishuCodex"
        display = self._assistant_name.upper()
        self.setWindowTitle(f"{display} — MARK XLIX")
        self._title_lbl.setText(display)
        if display in ("JARVIS", "J.A.R.V.I.S", "CYBERVISHUCODEX"):
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "CyberVishuCodex") or "CyberVishuCodex"
        self._log.append_log(f"SYS: Initialised. OS={os_name.upper()}. {self._assistant_name} online.")

class StartupAboutDialog(QDialog):
    """Small branded popup shown once when the app launches."""

    def __init__(self, assistant_name: str = "CyberVishuCodex", parent=None):
        super().__init__(parent)
        ...
        title.setStyleSheet(f"color: {C.PRI}; font-size: 22px; font-weight: 700; letter-spacing: 2px; border: none;")
        title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        lay.addWidget(title)

        subtitle = QLabel("Personal AI Assistant")
        subtitle.setStyleSheet(f"color: {C.TEXT_DIM}; font-size: 12px; letter-spacing: 1px; border: none;")
        ...
        info = QLabel(
            "Voice, vision, memory, and full computer control —\n"
            "built to run entirely on your own machine.\n\n"
            "Made with ❤ by CyberVishuCodex"
        )
        info.setWordWrap(True)
        info.setStyleSheet(f"color: {C.TEXT_MED}; font-size: 12px; border: none;")
...
class JarvisUI:
    def __init__(self, face_path: str, size=None):
        ...
        self._win.show()
        try:
            _name = _read_full_config().get("assistant_name", "CyberVishuCodex") or "CyberVishuCodex"
        except Exception:
            _name = "CyberVishuCodex"
        StartupAboutDialog(_name, parent=self._win).exec()
